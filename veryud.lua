--[[ ============================================================
  ...
  Usage:
    local SpeedyUI = loadstring(game:HttpGet(
      "https://raw.githubusercontent.com/xyraopsec/speedy-hub/master/SpeedyUI.lua?v=2.0"
    ))()
...
local SpeedyUI = {}
SpeedyUI.Version = "2.0"
print("[SpeedyUI] v2.0 loaded — proper dark glass, vivid red, readable text")
...
function SpeedyUI:CreateWindow(opts)
  ...
  -- ── Shell (main window) ──────────────────────────────────────
  -- Uses CanvasGroup so children clip-fade together on open/close
  local shell = New("CanvasGroup", {
    Name = "Shell",
    AnchorPoint = Vector2.new(0.5, 0.5),
    Position = UDim2.fromScale(0.5, 0.52),  -- start slightly low
    Size = UDim2.fromOffset(W * 0.9, H * 0.9),
  ...
  function Window:SelectTab(idx)
    ...
    for i, t in ipairs(self._tabs) do
      ...
      if on then
        pg.Visible = true
        pg.Position = UDim2.fromOffset(8, 0)
        tw(pg, 0.26, Enum.EasingStyle.Quad, Enum.EasingDirection.Out,
          { GroupTransparency = 0, Position = UDim2.fromOffset(0, 0) })
      else
        tw(pg, 0.14, nil, nil, { GroupTransparency = 1 })
        task.delay(0.15, function() if pg and pg.Parent then pg.Visible = false end end)
      end
    end
  end
  ...
  function Window:Toggle(force)
    ...
    if show then
      ...
      tw(shell, 0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out,
        { Size = self._curSize, GroupTransparency = 0, BackgroundTransparency = T.WinTrans })
    else
      tw(shell, 0.26, Enum.EasingStyle.Back, Enum.EasingDirection.In, {
        Size = UDim2.fromOffset(self._curSize.X.Offset * 0.94, self._curSize.Y.Offset * 0.94),
      })
      tw(shell, 0.22, nil, nil, { GroupTransparency = 1 })
      task.delay(0.3, function() if not self._open then self._gui.Enabled = false end end)
    end
  end
...
function SpeedyUI:Notify(o)
  ...
  local n = New("CanvasGroup", {
    ...
    BackgroundTransparency = 0.06,   -- near-solid so it's always readable
    BorderSizePixel = 0,
    GroupTransparency = 1,
  ...
  task.delay(dur, function()
    ...
    tw(n, 0.32, Enum.EasingStyle.Quad, Enum.EasingDirection.In,
      { GroupTransparency = 1, Position = UDim2.fromOffset(24, 0) })
