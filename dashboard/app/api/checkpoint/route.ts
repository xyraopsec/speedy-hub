...
export async function GET(req: NextRequest) {
  ...
  if (!token) {
    ...
    return NextResponse.json({ token: session.token, step: session.step });
  }
...
export async function POST(req: NextRequest) {
  ...
  if (elapsed < 3000 && stepCompleted > session.step) {
    return NextResponse.json({ error: "Please wait before proceeding." }, { status: 429 });
  }
  ...
  if (stepCompleted === 2 && session.step === 2) {
    ...
    if (session.keyIssued) {
      return NextResponse.json({ ok: true, step: 3, key: session.keyIssued });
    }
    ...
    return NextResponse.json({ ok: true, step: 3, key: newKey });
  }

  return NextResponse.json({ error: "Invalid step progression" }, { status: 400 });
}
